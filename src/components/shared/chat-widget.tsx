...
export function ChatWidget() {
  ...
  async function copyWechat() {
    await navigator.clipboard.writeText("IceKAI7109258");
    setCopied(true);
    ...
    <>
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 grid h-14 w-14 place-items-center rounded-2xl shadow-xl transition-all duration-300 hover:scale-110 active:scale-95 ${
          open ? "rotate-90 bg-slate-800 text-white" : "bg-gradient-to-br from-cyan-500 to-blue-600 text-white"
        }`}
        aria-label="在线客服"
      >
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-50 w-[340px] max-w-[calc(100vw-48px)] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl animate-in slide-in-from-bottom-4 duration-200">
          <div className="flex items-center gap-3 bg-gradient-to-r from-cyan-500 to-blue-600 px-5 py-4 text-white">
            ...
            <button onClick={() => setOpen(false)} className="ml-auto grid h-8 w-8 place-items-center rounded-lg transition-colors hover:bg-white/10">
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          {showWechat ? (
            <div className="p-5 text-center">
              <div className="relative overflow-hidden rounded-xl border border-slate-200 bg-slate-50 p-2">
                ...
                <img src="/customer-service-wechat.jpg" alt="客服企业微信二维码" className="mx-auto max-h-72 w-full object-contain" />
                <button
                  type="button"
                  onClick={() => setShowWechat(false)}
                  className="absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-lg bg-white/90 text-slate-600 shadow"
                  title="关闭二维码"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
              <p className="mt-3 text-sm font-black text-slate-700">扫码添加企业微信</p>
              <button type="button" onClick={copyWechat} className="mt-2 inline-flex items-center gap-1.5 text-xs font-bold text-cyan-600 hover:underline">
                <Copy className="h-3.5 w-3.5" />
                {copied ? "微信号已复制" : "复制微信号 IceKAI7109258"}
              </button>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-3 p-5">
              <button
                ...
                className="flex flex-col items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 p-4 text-center transition-all hover:-translate-y-0.5 hover:border-cyan-200 hover:bg-cyan-50"
              >
                ...
                <span className="text-[10px] font-semibold text-slate-400">扫码或复制微信号</span>
              </button>
              <a
                href="mailto:Belongstokk@protonmail.com"
                className="flex flex-col items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 p-4 text-center transition-all hover:-translate-y-0.5 hover:border-blue-200 hover:bg-blue-50"
              >
          ...
          <div className="border-t border-slate-100 px-5 py-3">
            <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 transition-all focus-within:border-cyan-400 focus-within:ring-3 focus-within:ring-cyan-400/10">
              <input placeholder="输入你的问题..." className="flex-1 bg-transparent text-sm font-medium text-slate-700 outline-none placeholder:text-slate-400" />
