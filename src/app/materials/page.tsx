...
import { allMaterialItems, materialCategories } from "@/lib/site-data";
...
export default async function MaterialsPage({
  ...
  let items = [...allMaterialItems];

  if (activeCategory !== "all") {
    const findCategory = (
      cats: typeof materialCategories,
      slug: string
    ): (typeof materialCategories)[number] | undefined => {
      for (const cat of cats) {
        if (cat.slug === slug) return cat;
        if (cat.children) {
          const found = findCategory(cat.children as typeof materialCategories, slug);
    ...
    const cat = findCategory(materialCategories, activeCategory);
    if (cat) {
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-xl">
        <div className="mx-auto flex h-14 w-[min(1500px,calc(100vw-28px))] items-center gap-4">
          ...
          <h1 className="text-base font-black text-slate-900 truncate">
            设计素材{activeCategory !== "all" ? ` · ${activeName}` : ""}
          </h1>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-3 mb-6">
            ...
            <div className="flex items-center gap-1 rounded-full border border-slate-200 bg-white p-1">
              <SortButton href={`/materials?category=${activeCategory}&sort=newest`} active={sortBy === "newest"}>最新</SortButton>
              <SortButton href={`/materials?category=${activeCategory}&sort=popular`} active={sortBy === "popular"}>最热</SortButton>
            </div>
          </div>
          ...
          {items.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-lg font-bold text-slate-600">没有找到匹配的素材</p>
              <p className="mt-2 text-sm text-slate-400">试试调整筛选条件或搜索关键词</p>
              <Link href="/materials" className="mt-4 inline-flex h-9 items-center rounded-full bg-cyan-500 px-5 text-sm font-bold text-white hover:bg-cyan-600 transition-colors">清除筛选</Link>
            </div>
          )}
